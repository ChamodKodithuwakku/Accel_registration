...
const columns = [
  ...
  { key: 'createdTime', label: 'Created Time', sortable: false },
  { key: 'lastUpdated', label: 'Last Updated', sortable: false },
]
...
function ActionButtons({ record, onEdit, onDelete, busyId }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        disabled={busy}
        className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs font-medium text-slate-700 shadow-sm transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 disabled:opacity-50"
      >
      ...
      <button
        ...
        disabled={busy}
        className="inline-flex items-center gap-1.5 rounded-lg border border-rose-200 bg-white px-2.5 py-1.5 text-xs font-medium text-rose-600 shadow-sm transition hover:border-rose-300 hover:bg-rose-50 disabled:opacity-50"
      >
        {busy ? (
    ...
    return (
      <div className="space-y-3 p-6">
        ...
        <p className="pt-2 text-center text-sm text-slate-500">
          <Spinner className="h-4 w-4 text-brand-600" label="Loading records..." />
        </p>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="px-6 py-14 text-center">
        ...
        <h3 className="mt-3 text-sm font-semibold text-slate-900">Could not load records</h3>
    ...
    return (
      <div className="px-6 py-16 text-center">
        ...
        <h3 className="mt-3 text-sm font-semibold text-slate-900">
          {hasFilter ? 'No matching records' : 'No registrations yet'}
        </h3>
        <p className="mt-1 text-sm text-slate-500">
          {hasFilter
            ? 'Try a different name, student ID or phone number.'
            : 'New submissions from the registration form will appear here.'}
        </p>
      </div>
    )
  }
            ...
            {records.map((record) => (
              <tr key={record.id} className="transition hover:bg-slate-50/70">
                ...
                <td className="px-4 py-3">
                  <span className="rounded-md bg-brand-50 px-2 py-0.5 font-mono text-xs font-medium text-brand-700">
                    {record.studentId}
                  </span>
                </td>
                ...
                <td className="px-4 py-3 text-slate-600">{record.createdDate || '-'}</td>
                <td className="px-4 py-3 text-slate-600">{record.createdTime || '-'}</td>
                <td className="px-4 py-3 text-slate-500">{record.lastUpdated || '-'}</td>
                <td className="px-4 py-3">
                  <div className="flex justify-end">
                    <ActionButtons record={record} onEdit={onEdit} onDelete={onDelete} busyId={busyId} />
                  </div>
                </td>
              </tr>
            ))}
          ...
          <li key={record.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 font-mono text-xs text-brand-700">{record.studentId}</p>
              </div>
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
              ...
              <div className="col-span-2">
                ...
                <dd className="mt-0.5 text-slate-700">{record.lastUpdated || '-'}</dd>
              </div>
            </dl>
